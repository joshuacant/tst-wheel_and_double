...
browser.runtime.onMessageExternal.addListener((aMessage, aSender) => {
    if (aSender.id === kTST_ID) {
        ...
        switch (aMessage.type) {
            ...
            case 'tab-clicked':
                if (doubleClickEnabled === false) {
                    break;
                }
                const d = new Date();
                const currentClickTime = d.getTime();
                const clickDelta = currentClickTime - previousClickTime;
            ...
            case 'scrolled':
                ...
                let activeTabPosition = null;
                for (let iTab = 0; iTab < aMessage.tabs.length; ++iTab) {
                ...
                let tabDelta = null;
                let validTabFound = true;
                let mouseDelta = aMessage.deltaY;
                    ...
                    if (activeTabPosition === aMessage.tabs.length - 1) {
                        ...
                        while (validTabFound === false) {
                            ...
                            if (activeTabPosition + tabDelta > aMessage.tabs.length - 1) {
                                if (skipCycling === true && skipCollapsed === true) {
                                    tabDelta = 0;
                                }
                                else {
